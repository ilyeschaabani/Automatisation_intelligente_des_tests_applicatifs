...
export async function GET(request: Request) {
  ...
  const upstream = await fetch(`${MS_EXECUTION_SERVICE_URL}/api/functional-evaluation${url.search}`, {
    method: 'GET',
...
export async function POST(request: Request) {
  ...
  const upstream = await fetch(`${MS_EXECUTION_SERVICE_URL}/api/functional-evaluation`, {
    method: 'POST',
    headers: buildHeaders(request, cookieStore.toString()),
    body,
  })

...
export async function POST(
  request: Request,
  context: { params: Promise<{ id: string; discoveryId: string }> | { id: string; discoveryId: string } },
) {
  const cookieStore = await cookies()
  const url = new URL(request.url)
  const resolved = await context.params
  const id = resolved.id
  const discoveryId = resolved.discoveryId

  const bodyText = await request.text().catch(() => '')

  const upstream = await fetch(
    `${TEST_MANAGEMENT_SERVICE_URL}/api/projects/${encodeURIComponent(id)}/discoveries/${encodeURIComponent(discoveryId)}/complete${url.search}`,
    {
      method: 'POST',
      headers: {
